use serde::Serialize;
use std::time::Duration;
use tauri::{async_runtime, Emitter};
use tokio::time::sleep;
use windows::Media::Control::GlobalSystemMediaTransportControlsSessionManager;
mod lyrics;

// struct to send media as json to frontend
#[derive(Clone, Serialize)]
struct Media {
    artist: String,
    title: String,
    album: String,
    duration: f64,
    lyrics: String,
}

// Learn more about Tauri commands at https://tauri.app/develop/calling-rust/
#[tauri::command]
async fn fetch_current_media(app: tauri::AppHandle) {
    async_runtime::spawn(async move {
        // event has to match with frontend
        let event = "fetch_current_media";

        // initialize media manager
        let manager_result = GlobalSystemMediaTransportControlsSessionManager::RequestAsync();
        if manager_result.is_err() {
            let _ = app.emit(event, "Failed to request media manager");
            return;
        }
        let manager = match manager_result.unwrap().await {
            Ok(m) => m,
            Err(_) => {
                let _ = app.emit(event, "Failed to get session manager");
                return;
            }
        };

        let mut last_track = String::new();

        loop {
            // get current media
            if let Ok(session) = manager.GetCurrentSession() {
                if let Ok(media) = session.TryGetMediaPropertiesAsync() {
                    if let Ok(media_props) = media.await {
                        let artist = media_props
                            .Artist()
                            .unwrap_or("Unknown Artist".into())
                            .to_string_lossy();
                        let title = media_props
                            .Title()
                            .unwrap_or("Unknown Title".into())
                            .to_string_lossy();
                        let current_track = format!("{} - {}", artist, title);

                        // fetch lyrics only if track changed
                        if current_track != last_track {
                            last_track = current_track.clone();

                            // calculate duration
                            let timeline = session.GetTimelineProperties().unwrap();
                            let duration = (timeline.EndTime().unwrap().Duration
                                - timeline.StartTime().unwrap().Duration)
                                as f64
                                / 10_000_000.0;

                            // fetch lyrics and emit if found
                            if let Some((lyrics_response, lyrics)) =
                                lyrics::fetch_lyrics(artist.clone(), title.clone(), duration).await
                            {
                                let _ = app.emit(
                                    event,
                                    Media {
                                        artist: lyrics_response.artist_name.unwrap(),
                                        title: lyrics_response.track_name.unwrap(),
                                        album: lyrics_response.album_name.unwrap(),
                                        duration: lyrics_response.duration.unwrap(),
                                        lyrics,
                                    },
                                );
                            }
                        }
                    }
                }
            }
            sleep(Duration::from_millis(100)).await;
        }
    });
}

#[cfg_attr(mobile, tauri::mobile_entry_point)]
pub fn run() {
    tauri::Builder::default()
        .plugin(tauri_plugin_opener::init())
        .invoke_handler(tauri::generate_handler![fetch_current_media])
        .run(tauri::generate_context!())
        .expect("error while running tauri application");
}
